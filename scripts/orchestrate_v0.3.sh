...
mkdir -p logs prompts workspace memory

TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
REPORT="logs/final_report_$TIMESTAMP.md"

echo "========================================"
echo "Hermes Coding Room Orchestrator v0.3"
echo "========================================"
echo ""
echo "요청:"
echo "$TASK"
...
$(cat logs/test.log)"
  ...
  {
    echo "## Codex 수정 로그 attempt=$((ATTEMPT + 1))"
    echo '```text'
    cat logs/codex.log
    echo '```'
    echo ""
...
if [ $SUCCESS -eq 1 ]; then
  ...
  ./scripts/ask_claude.sh "너는 리뷰어 역할이다. 절대 파일을 생성하거나 수정하지 마라. 현재 작업은 테스트에 성공했다. workspace와 logs/test.log 기준으로 최종 리뷰와 개선점만 텍스트로 알려줘."
