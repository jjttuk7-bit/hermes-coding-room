...
CONTEXT_FILE=$(./scripts/build_context.sh)
...
echo "========================================"
echo "Hermes Coding Room Orchestrator v0.5"
echo "========================================"
echo ""
...
./scripts/ask_claude.sh "너는 설계자/리뷰어 역할이다. 절대 파일을 생성하거나 수정하지 마라. 실제 구현은 Codex가 담당한다. 아래 작업실 기억을 참고하되, 현재 요청에 필요한 내용만 반영하라. 다음 작업의 구현 계획과 검증 기준만 짧고 명확하게 제안해줘.\n\n[작업실 기억]\n$CONTEXT\n\n[현재 작업]\n$TASK"
...
./scripts/ask_codex.sh "아래 작업실 기억을 참고하되, 현재 작업 요청을 최우선으로 수행해라. 다음 작업을 실제 파일로 구현해줘. 설명만 하지 말고 반드시 요청된 파일을 생성하거나 수정해.\n\n[작업실 기억]\n$CONTEXT\n\n[현재 작업 요청]\n$TASK"
...
while [ $ATTEMPT -le $MAX_RETRY ]; do
  ...
  ./scripts/ask_codex.sh "테스트가 실패했다. 아래 작업실 기억과 테스트 로그를 보고 문제를 수정해줘. 반드시 실제 파일을 수정하고 같은 검증 명령이 성공하게 만들어라.\n\n작업실 기억:\n$CONTEXT\n
